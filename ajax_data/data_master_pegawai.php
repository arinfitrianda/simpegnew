...
$columns = array(
    ...
    array('db' => '`d`.`nama_dept`','dt' => 'nama_dept', 'field'=>'nama_dept', 'as'=>'nama_dept'),
);
 ...
 */
 ...
 $joinQuery = "FROM `pegawai` AS `peg` INNER JOIN `departemen` AS `d` ON(`peg`.`id_depart`=`d`.`id_dept`)";
 if($id_dept!=0){
   $extraWhere = " `d`.`id_dept`='".$id_dept."'";
 }else{
   $extraWhere = "";
 }
 $groupBy = "";
